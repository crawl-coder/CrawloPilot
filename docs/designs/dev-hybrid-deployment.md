...
4. CrawloPilot 创建通道（先添加服务器，再建通道）
   ├── SSH 通道: host=117.72.16.51, port=22
   ├── Docker 通道: host=127.0.0.1, port=2376 (隧道)
   └── Agent 通道: 创建后拿 token，在云服务器运行 agent（或一键部署）
```
