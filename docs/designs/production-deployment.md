...
`/opt/crawlopilot/workspace/{task_id}/`，远程执行 `python main.py`，
轮询进程状态并拉取日志。
...
（基础镜像 `crawlopilot/base:1.7.2` + 爬虫代码），在节点上启动容器执行。
...
流程：注册（携带 token）→ 心跳保活 → 领取任务 → 下载代码 → 执行 → 上报日志。
...
然后在 CrawloPilot 中为每台节点创建 SSH 节点，私钥都填同一份（A 的私钥）。
...
| 管理服务器可用性 | 单实例 | Scheduler 主备（Redis 分布式锁）+ 共享存储 |
