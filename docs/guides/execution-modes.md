...
> 本评价基于 V1 实测（2026-08-24）与源码审查，而非文档描述。

### 一句话总评

> **骨架优秀、局部出血：本地是好的演示默认，Docker 是当下生产主力，SSH 是体面的兼容项，Agent 是必须重注投入的战略路径——而它恰好是刚修完大出血的那个。**

### 量化速记

| 维度 | 本地 | SSH | Docker | Agent |
|---|---|---|---|---|
| 开箱体验 | ★★★★★ | ★★★☆ | ★★★☆ | ★★☆ |
| 环境/依赖隔离 | ☆（无隔离） | ★★☆（venv 尽力） | ★★★★★ | ★★★★（每任务 venv） |
| 安全面 | 小 | **大**（整机登录凭据） | **大**（daemon ≈ root） | 小（token 权限极小） |
| 运行健壮性 | ★★☆ | ★★★ | ★★★★ | ★★☆ |
| 单节点并发 | 多任务并行 | 取决于配置 | 取决于配置 | **1，串行** |
| 实测状态 | 最稳（25/25） | 未实测 | 真机验证已过 | 修复后全链路打通 |

### 各模式定位

**本地** — 演示与验收的理想默认，控制面重启僵尸任务（F1）和无依赖隔离是真实生产化前提。

**SSH** — 务实的兼容选项（有旧服务器时零 daemon 即用），但安全暴露面最大且实时性一般，已转维护模式，不再新投。

**Docker** — 工程完成度最高：隔离彻底、镜像复用秒级（6.1s）、项目 Dockerfile 优先的策略聪明；前提是 Docker daemon 可达，生产建议 socket 同机或 Agent 化规避 2375 暴露。

**Agent** — 战略上最重要（V2 多机分布式深爬的关键路径）同时工程上最薄弱：F5/F6/F7 集中暴露，单 agent 串行主循环限制并发扩展，Wave A 必须兑现（venv 缓存、版本握手、协议回归）。
