...
因此 V1 裁剪掉了 Git 管理、调度系统、监控告警、数据质量、代理池、API 管理、操作审计等
"锦上添花"的功能——它们大多依赖外部组件（Celery Worker、Prometheus、通知渠道）或本身就是半成品。
裁剪不是否定这些功能，而是**让第一版可被完整验证**。数据库表保留，V2 可平滑恢复。
...
| 移除调度系统 | 原实现依赖 Celery Worker（无 Worker 时任务永远不跑），且非 V1 目标 |
...
4. **V2 顺序**：调度（进程内 APScheduler + LocalExecutor）→ 监控告警 → Git 管理 → 数据质量 → 代理池/API 管理 → 审计
