...
启动 → register（注册，换取 node_id）
     ...
     → poll_task 长轮询（服务端最长挂起 25s，空闲约 5s 一次）
        ├─ 无任务 → 继续轮询
...
节点管理 → 添加节点 → 先选所属服务器，连接方式选「Agent 代理」→ 创建后复制注册令牌。

### 2. 部署并启动 Agent（两种方式任选）

**方式 A：平台一键部署（推荐，适合多台服务器）**

```text
服务器下先创建 SSH 通道 → 节点管理 → 「批量部署 Agent」→ 勾选服务器
→ 平台自动：上传 crawlo_agent.py + 写 systemd（开机自启/崩溃重启）+ 启动 + 等上线
```

**方式 B：手动部署（单台或无法复用 SSH 时）**
